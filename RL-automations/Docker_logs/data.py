...
for container in containers:
    ...
    for i in container_logs.split('\n'):
        if any(re.search(pattern.rstrip(), i) for pattern in config_file):
            ...
            #############################
            # actual_time=datetime.strptime(time_string, '%Y-%m-%d %H:%M:%S.%f')
            # #print(actual_time)
            # actual_time=actual_time.strftime('%Y-%m-%d %I:%M:%S')
            # print("time",actual_time)
            # print("past",past_time)
            # print("current", current_time)

                if str(current_time) > str(actual_time) > str(past_time):
                    demo1=f"<tr><td>{container.name}</td><td>{i.split(' ')[1]}</td><td>{i.split(' ')[3]}</td><td>{' '.join(i.split(' ')[4:])}</td>"
                    print("demo1 line",demo1)
                # if error_code not in config_file:
                # #print(i)
                #     # demo=f"<tr><td>{container.name}</td><td>{i.split(' ')[1]}</td><td>{i.split(' ')[3]}</td><td>{' '.join(i.split(' ')[4:])}</td>"

                #     # l3.append(demo)
                #     print("end")
                else:
                # demo=f"<tr><td>{container.name}</td><td>{i.split(' ')[1]}</td><td>{i.split(' ')[2]}</td><td>{i.split(' ')[3:]}</td>"
#demo=f"<tr><td>{container.name}</td><td>{i.split(' ')[1]}</td><td>{i.split(' ')[2]}</td><td>{' '.join(i.split(' ')[3:])}</td>"
                # l3.append(demo)
                    print("inside else")
            except:
                pass
